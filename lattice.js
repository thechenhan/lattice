...
function fillScene() {
	...
	for (var i = 0; i <= 10; i++) {
		for (var j = 0; j <= 10; j++) {
			for (var k = 0; k <= 2; k++) {
				var unit = createUnitCell(cylLength, cylinderMaterial, theta);
				scene.add( unit );
				unit.position.x = i;
				unit.position.y = j;
				unit.position.z = k;
...
function createUnitCell(cylLength, cylinderMaterial, theta){

	
	var unitCell = new THREE.Object3D(); //this is the X shape unit cell;
	
	var cylinderGeo = new THREE.CylinderGeometry( 0.1, 0.1, cylLength, 32 );
	var ballGeo = new THREE.SphereGeometry( 0.1, 32, 32 );
	...
	for ( var i = 0; i < 4; i++ ) {
    	...
    	var ballend = new THREE.Mesh( ballGeo, cylinderMaterial );
    	var ballend2 = new THREE.Mesh( ballGeo, cylinderMaterial );

    	ballend.position.y = cylLength / 2;
    	ballend2.position.y = - cylLength / 2;

    	var strut = new THREE.Object3D();
		strut.add(cylinder);
		strut.add(ballend);
		strut.add(ballend2);
    	...
    	strut.matrixAutoUpdate = false;

    	strut.matrix.makeRotationAxis( rotationAxis, theta );
